	...
	)
{
	...
	else if (matLight._14 == 2.0f)
	{
		float3 lightPosition = float3(matLight._11, matLight._12, matLight._13);
		float3 dir = float3(matLight._21, matLight._22, matLight._23);
		float maxRange = matLight._41;
		float minRange = matLight._42;
		float minAngle = matLight._43;
		float maxAngle = matLight._44;
		float DistancePow = matLight._24;
		...
		//광원 량....
		float NdotL = dot(lightDir, normal);//거리
		float diff = saturate(NdotL);
		...
		//만약 Spot 이라면....
		//박진한 스폿라이트 추가 한 것
		float3 dDir = normalize(position - lightPosition);
		float3 lDir = normalize(dir);
		float dAngle = abs(acos(dot(lDir, dDir)));
		
		if (dAngle < minAngle)
			t = 1;
		else if (dAngle > maxAngle) 
			t = 0;
		else
		{
			t = saturate((dAngle - minAngle) / (maxAngle - minAngle));
			t = 1.0f - t;
		}
