...
{   
   ...
   float diff2 = dot(worldNormal, normalize(-worldLightDir2.xyz));
   //if( diff < 0.0f )
   //  diff = saturate( abs( diff ) * 0.3f + 0.1f );
   diff2 = saturate(diff2);
   
   diff = max(diff, 0.30);
   diff2 = max(diff2 * 0.5, 0.30);
   ...
   float3 fc1 = (finalColor * worldLightColor.xyz) * diff;
   float3 fc2 = (finalColor * worldLightColor2.xyz) * diff2;

   Output.baseColor = float4(fc1 + fc2, 1);
