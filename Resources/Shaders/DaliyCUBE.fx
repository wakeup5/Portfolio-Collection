...
samplerCUBE DaySampler = sampler_state
{
   Texture = Day_Texture;
   MAGFILTER = LINEAR;
   MINFILTER = LINEAR;
...
samplerCUBE NightSampler = sampler_state
{
	Texture = Night_Texture;
	MAGFILTER = LINEAR;
	MINFILTER = LINEAR;
...
{   
	float4 day = texCUBE(DaySampler, normalize(Input.Normal));
	float4 night = texCUBE(NightSampler, normalize(Input.Normal));

	return (day * Day_Alpha) + (night * (1 - Day_Alpha));
}
