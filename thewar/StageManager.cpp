...
void StageManager::update()
{
	...
	if (_isStageInit && _ui->getKillNum() <= 0)
	{
		_isStageInit = false;
		_stageNum++;
		if (_stageNum >= 5) _stageNum = 0;
	}

	if (_playerManager->getPlayer()->getHp() <= 0 || _playerManager->getQueen()->getHp() <= 0)
	{
...
void StageManager::stage4()
{
	...
	_campfire->initialize(IMAGEMANAGER->findImage("campfire")->getSpriteImage(6, 1), STAGE_WIDTH / 2 - 50, STAGE_HEIGHT - LAND_HEIGHT, 40, 50);
	_fountain->initialize(IMAGEMANAGER->findImage("fountain")->getSpriteImage(3, 1), STAGE_WIDTH / 2 + 50, STAGE_HEIGHT - LAND_HEIGHT, 50, 66);

	_stage = new Stage;
	_stage->initialize(_bg, _land, _campfire, _fountain);

	_playerManager->getPlayer()->initialize();
	_playerManager->getPlayer()->setPosition(STAGE_WIDTH / 2, STAGE_HEIGHT - 500);
	_playerManager->getQueen()->initialize();
	_playerManager->getQueen()->setPosition(STAGE_WIDTH / 2, STAGE_HEIGHT - LAND_HEIGHT);
	_playerManager->getQueen()->setSpeed(0);
